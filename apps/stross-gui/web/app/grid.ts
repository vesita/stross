...
/** 本机在线共享缓存（供本机卡片流区渲染）。 */
let localStreamsCache: RemoteStream[] = [];
...
async function autoNegotiateMic(
  dev: Pick<DeviceView, 'base'>,
): Promise<{ ok: boolean; error?: string; token?: string; streamId?: string }> {
  if (!dev.base) return { ok: false, error: '设备基址不可用' };
  const negBase = negotiatorBase(dev.base);
  if (!negBase) return { ok: false, error: '设备基址解析失败' };
  let ident: DeviceIdentity;
  try {
    ident = (await call('device_identity')) as DeviceIdentity;
  } catch (e) {
    return { ok: false, error: '无法读取本机身份：' + (e as Error).message };
  }
  ...
  try {
    ...
    const grant = (await resp.json()) as ShareGrant;
    if (!grant.token || !grant.streamId) {
      return { ok: false, error: '协商响应缺少凭证字段' };
    }
    return { ok: true, token: grant.token, streamId: grant.streamId };
  } catch (e) {
    if (ctrl.signal.aborted) return { ok: false, error: '等待对方确认超时（15s）' };
    return { ok: false, error: '无法连接设备协商端点：' + (e as Error).message };
  } finally {
    clearTimeout(timer);
  }
}
...
async function openMicShare(dev: DeviceView): Promise<void> {
  ...
  try {
    ...
    if (iresp.ok) {
      const info = (await iresp.json()) as { quicPort?: number };
...
async function startMicShare(): Promise<void> {
...
async function startMicShareWith(p: {
  ...
  try {
    ...
    if (micShare) micShare.active = true;
    ...
    const errBox = $('mic-error');
    errBox.textContent = '推流启动失败：' + (e as Error).message;
    errBox.classList.remove('hidden');
    $('mic-status').textContent = '';
...
async function stopMicShare(): Promise<void> {
  try {
    await call('stop_stream');
  } catch (_) { /* ignore */ }
  if (micShare) micShare.active = false;
  setMicRunning(false);
  $input('mic-token-input').disabled = false;
...
async function pollMicShareStatus(): Promise<void> {
  if (!micShare || !micShare.active) return;
  // capture_status 反映 Android 原生采集真实状态（micOnly 授权失败会回传错误）
  if (IS_ANDROID) {
    try {
      const cs = (await call('capture_status')) as CaptureStatus;
      if (cs.error) {
  ...
  const st = (await call('stream_status').catch(() => null)) as StreamStatus | null;
  if (st && !st.running) {
    if (micShare) micShare.active = false;
    setMicRunning(false);
    $input('mic-token-input').disabled = false;
...
function setMicRunning(r: boolean): void {
...
async function startMicReceive(): Promise<void> {
  const btn = $btn('mic-recv-btn');
  setBtnLoading(btn, true);
  hideGridError();
  try {
    const v = (await call('issue_share_token', { ttlSecs: 600 })) as ShareTokenView;
    ...
    setBtnLoading(btn, false);
    showGridError('签发凭证失败：' + (e as Error).message);
  }
}

/** 到期时间倒计时文案（Unix 秒 → "约 N 分钟"）。 */
function fmtSecs(expiresAt: number): string {

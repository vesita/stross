...
if (filterInp) {
  ...
  check('设备搜索过滤生效（本机 + 匹配的手机A 共 2 张卡片）', cardsFiltered.length === 2, `实际 ${cardsFiltered.length}`);
...
console.log('\n[14] 管理界面与消费界面分离（View Switcher & Panels）验证');
const switcher = $('view-switcher');
check('全局视图切换导航栏存在', !!switcher);
...
check('设备管理模式按钮存在', !!btnManage);
check('消费播放台模式按钮存在', !!btnConsume);
check('管理视区面板与消费视区面板均存在', !!panelManage && !!panelConsume);
btnConsume?.click();
await sleep(50);
check('点击消费播放台按钮后消费面板激活', panelConsume?.classList.contains('active') && !panelManage?.classList.contains('active'));
check('消费播放台按钮处于 active 态', btnConsume?.classList.contains('active') && !btnManage?.classList.contains('active'));
btnManage?.click();
await sleep(50);
check('点击设备管理按钮后管理面板激活', panelManage?.classList.contains('active') && !panelConsume?.classList.contains('active'));
check('设备管理按钮处于 active 态', btnManage?.classList.contains('active') && !btnConsume?.classList.contains('active'));

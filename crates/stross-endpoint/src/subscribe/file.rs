//! 文件**订阅端点**（File 能力族的订阅端；docs/framework-v3.md §3）：
//! 由内核从注册表 `(节点, 端点, 策略)` 解析后**生成**（订阅端点生成），
//! 经 [`crate::contract::FileHost::receive_file`] 把订阅的文件流落盘到 `out_dir`。
//!
...
impl crate::contract::SubscribeEndpoint for FileReceiveEndpoint {
    fn subscribe(
        ...
        // `host` 是 SubscribeHost（MediaHost + FileHost 组合）：文件订阅端
        // 只用 FileHost 部分（receive_file）。

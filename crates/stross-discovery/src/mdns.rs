//! mDNS 服务发现实现：中继/推流端用 `_stross._tcp` 广播自己，局域网内其它
//! 设备可以发现中继入口（[`MdnsDiscovery::browse`] 浏览 / [`MdnsDiscovery::start`]
//! 广播）。
...
//!
//! 本模块迁自 stross-kernel `discovery/mdns.rs`（v3 P2b，docs/framework-v3.md
//! §3.8）：实现 struct 原名 `Discovery`，与契约 trait 同名冲突，改名为
//! [`MdnsDiscovery`]；方法面与行为逐行保留。
...
/// mDNS 广播句柄。
///
/// 保存注册时的完整参数，供 [`MdnsDiscovery::redefine`] 保持同 fullname **覆盖更新**
...
impl MdnsDiscovery {
    ...
    /// 名——发现层零 OS 服务调用，见 docs/framework-v3.md 平台无关红线；
    /// 取不到时调用方可传 "stross" 兜底）。
    pub fn start(
        instance: &str,
        ips: &[IpAddr],
...
fn select_reachable_ip(reachable: &[IpAddr]) -> Option<&IpAddr> {
    select_reachable_ip_from(&stross_transport::net::local_ips(), reachable)
}

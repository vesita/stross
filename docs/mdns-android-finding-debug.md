...
3. **可选的代码兜底**：mDNS 增加"单播兜底"——手机公告同时单播下发（或对端对手机查询回单播），让手机在**收不到下行多播**时仍能被发现/发现对端。非必须（根因是环境）；后续如需 WiFi 下也稳定再实现。
...
- **待办**：P0-1 真机闭环建议走 USB 网络（已可发现）；WiFi 侧按 8.3-1 改路由器或按 8.3-3 实现单播兜底。

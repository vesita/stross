//! 局域网节点扫描聚合（mDNS 结果 + HTTP 探测 + 视图模型）——**发现实现层**。
//!
//! 分层（docs/framework-v3.md §3.8）：扫描聚合对 CLI `devices`、`adb status`
//! 与 GUI 设备卡片是同一份逻辑——收敛在此，壳层只做**格式化/渲染**（中文
//! 标签、卡片布局各自完成，本模块只出结构化数据）。
//!
//! 输入：[`MdnsDiscovery::browse`] 原始结果 + 本机 IP + 探测超时；
//! 输出：[`ScannedNode`] 列表（已去重、已过滤、已排序、已探测）。
//!
//! 本模块迁自 stross-kernel `discovery/aggregate.rs`（v3 P2b）；`ScannedNode` /
//! `StreamView` / `to_views` 用契约 crate（lib.rs）定义，`DiscoveryResp` 用
//! `stross-view`（展示 DTO），删除 kernel 侧同名重复定义。
...
use stross_proto::message::{DiscoveryInfo, StreamInfo};
use stross_transport::net::{is_fake_or_link_local, local_ips};
use stross_view::DiscoveryResp;

use crate::{Discovered, MdnsDiscovery, ScannedNode, to_views};

/// 发现权威端口：协商/发现服务监听于此（桌面与 Android GUI 一致），
/// 是 mDNS 与子网扫描共同收敛的节点入口。**与协商端口是同一端口**，真源在
/// [`stross_view::ports::NEGOTIATOR_DISCOVERY`]（此处仅别名，消除局部重复定义）。
pub use stross_view::ports::NEGOTIATOR_DISCOVERY as DISCOVERY_PORT;

/// 进程内全局复用的 HTTP 探测客户端（连接池；子网扫描并发高，避免每次新建）。
...
/// `/api/info` 探测（中继入口各传输端口；扫描聚合只取 SRT/QUIC 端口）。
///
/// 分层说明：中继 `/api/*` 的响应契约**单一真源**在 stross-kernel relay
/// （server 侧 DTO + 官方客户端）；本 crate 因分层不可反向依赖 kernel，
/// 故此处用只读投影结构解析同一 wire 格式（字段与 `RelayInfoResp` 一致，
/// 仅保留扫描需要的两个端口）。不可达 / 非 2xx / 解析失败返回 `None`
/// （调用方按「离线」处理，与原 `relay_http::info` 的 Err 语义等价）。
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
struct RelayProbeInfo {
    srt_port: Option<u16>,
    quic_port: Option<u16>,
}

/// `/api/streams` 响应：兼容裸数组（现行服务端形态）与
/// `{ "streams": [...] }` 包裹形态（历史兼容；与 kernel relay client 双形态一致）。
#[derive(Debug, Deserialize)]
#[serde(untagged)]
enum StreamsResp {
...
/// 探测中继 `/api/info`；不可达 / 非 2xx / 解析失败 = `None`（离线）。
async fn probe_info(host: &str, port: u16, timeout: Duration) -> Option<RelayProbeInfo> {
    let url = format!("http://{host}:{port}/api/info");
...
/// 探测中继 `/api/streams`（两种响应形态统一展开）；失败 = `None`。
async fn probe_streams(host: &str, port: u16, timeout: Duration) -> Option<Vec<StreamInfo>> {
    let url = format!("http://{host}:{port}/api/streams");
    let resp = HTTP_CLIENT.get(&url).timeout(timeout).send().await.ok()?;
    if !resp.status().is_success() {
        return None;
    }
    resp.json::<StreamsResp>().await.ok().map(StreamsResp::list)
}
...
pub async fn scan(
    ...
    for d in found {
        ...
        if let Some(resp) = probe_info(&probe_ip, d.port, probe).await {
            node.online = true;
            node.srt_port = resp.srt_port;
            node.quic_port = resp.quic_port;
        }
        if let Some(list) = probe_streams(&probe_ip, d.port, probe).await {
...
/// 手动地址并入。
///
/// 分层（docs/framework-v3.md）：browse + `local_ips` + 去重合并
/// 全部收敛在此，壳层（CLI 命令 / Tauri 命令）只做参数转译，禁止各自拼装
/// 聚合流程（曾出现在 GUI `scan_devices` 命令层）。
///
/// `browse` mDNS 浏览窗口；`probe` 每节点 HTTP 探测超时（下限 100ms，避免
/// 壳层误传小值打爆网络）；`extra_base_urls` 手动添加的地址（无 mDNS），
/// 一并探测并入（按 `ip:port` 去重）。
///
/// **mDNS 失效回退**：当 mDNS 浏览未发现任何**远端**节点（路由只掐下行多播、
...
pub async fn scan_lan(
    ...
    let found = MdnsDiscovery::browse(browse).await?;
...
/// [`DiscoveryResp`] 即以其中继入口端口构造 [`ScannedNode`]（在线）；
...
async fn scan_probe_host(host: &str, probe: Duration) -> Option<ScannedNode> {
    ...
    if let Some(resp) = probe_info(host, node.port, fast).await {
        node.online = true;
        node.srt_port = resp.srt_port;
        node.quic_port = resp.quic_port;
    }
    if let Some(list) = probe_streams(host, node.port, fast).await {
        node.streams = to_views(list);
    }
    Some(node)
}
...
pub async fn probe_base(base: &str, probe: Duration) -> Option<ScannedNode> {
    ...
    if let Some(resp) = probe_info(&host, port, probe).await {
        node.online = true;
        node.srt_port = resp.srt_port;
        node.quic_port = resp.quic_port;
    }
    if let Some(list) = probe_streams(&host, port, probe).await {
        node.streams = to_views(list);
    }
    Some(node)
}
...
mod tests {
    ...
    async fn scan_dedupes_by_instance_preferring_ipv4() {
        let txt = DiscoveryInfo {
            ...
            name: "电脑".into(),
            roles: vec![stross_proto::message::RoleId::Sender],
            media: vec![stross_proto::message::MediaKind::Screen],
            transports: vec![stross_proto::message::TransportId::Ws],
            codecs: vec![],
            endpoints: vec![],
        }
        .to_txt()

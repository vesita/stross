...
//! * pick 规则层：装载/解读语义迁出为概念 crate `stross-serialize`（Loader /
//!   loader_for）与 `stross-pick`（Interpreter / RealtimePacing / StrictOrdered /
//!   JitterBuffer / InterpretRegistry），内核只消费其 trait
//!   （docs/framework-v3.md §3.6 / §3.7）
//! * [`sender`] / [`watch`]：推流客户端与观看链路
//! * mDNS 服务发现（浏览 + 广播）实现单一真源在概念 crate `stross-discovery`
//!   （v3 P2b 迁出；内核薄壳 P3 删除，壳层直连 stross-discovery，
//!   docs/framework-v3.md §3.8）
...
pub use kernel::{
    ...
    FileSource, Kernel, KernelEvent, NodeEntry, NodeInfo, NodeRegistration, NodeRole,
    PinAuthPolicy, RelayDataPlane, Session, SessionPrefs, SubscribeEndpointFactory, TransportAddr,
    UnifiedRegistry, id::Id,
};
pub use negotiator::{
...
pub use receiver::{ReceiveLinkView, Receiver};
...
pub use stross_view::{
    ...
    PendingRequestsPayload, ReceiveStats, RelayInfo, SessionCreatedView, SessionView,
    SessionsPayload, ShareTokenView, StartResult, StatusView, StoppedView, StreamStatus,
    TeardownView, UnpublishedView,
};
pub use subscriber::{
...
pub use stross_view::ports::{QUIC as DEFAULT_QUIC_PORT, SRT as DEFAULT_SRT_PORT};

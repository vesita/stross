...
function renderDeviceList() {
    ...
    if (!deviceViews.length) {
        box.appendChild(emptyState('wifi', '未发现局域网其它节点', '请确保节点连接同一 Wi-Fi 并已开启「可被发现」，或在上方手动输入 IP 添加。', {
            label: '重新扫描',
            icon: 'refresh',
    ...
    const filtered = deviceFilterQuery
        ? deviceViews.filter((d) => d.name.toLowerCase().includes(deviceFilterQuery) ||
            d.meta.toLowerCase().includes(deviceFilterQuery))
        : deviceViews;
    if (deviceFilterQuery && !filtered.length) {
        box.appendChild(emptyState('filter', '未找到匹配的节点', `未找到与「${deviceFilterQuery}」匹配的节点名称或 IP 地址`, {
            label: '清除搜索',
            icon: 'x',
            onClick: () => {
...
/** 选中节点后同步更新右侧订阅工作台顶栏。 */
function updateStageForDevice(dev) {
    selectedDevice = dev;
    const title = $('stage-title');
    const sub = $('stage-sub');
    const avatar = $('stage-avatar');
    if (title)
        title.textContent = `订阅 · ${dev.name}`;
    if (sub)
        sub.textContent = `${dev.meta} · 在线可订阅接收`;
    if (avatar) {
        const isPhone = /android|phone|手机/i.test(dev.name) || /android/i.test(dev.meta);

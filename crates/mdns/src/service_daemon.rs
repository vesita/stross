...
use crate::{
    Receiver, ResolvedService, TxtProperties, current_time_millis,
    dns_cache::{DnsCache, IpType},
    dns_parser::{
        CLASS_CACHE_FLUSH, CLASS_IN, DnsAddress, DnsEntryExt, DnsIncoming, DnsOutgoing, DnsPointer,
        DnsRecordBox, DnsRecordExt, DnsSrv, DnsTxt, FLAGS_AA, FLAGS_QR_QUERY, FLAGS_QR_RESPONSE,
        InterfaceId, MAX_PKT_ABSOLUTE_IPV6, MAX_PKT_DEFAULT, RRType, ScopedIp, ip_address_rr_type,
        max_pkt_absolute,
    },
    error::{Error, Result, e_fmt},
    service_info::{
        DnsRegistry, MULTICAST_RATE_LIMIT_MILLIS, MyIntf, Probe, ServiceInfo, ServiceStatus,
        valid_ip_on_intf,
    },
};
use flume::{Sender, TrySendError, bounded};
use if_addrs::{IfAddr, Interface};
use mio::{Interest, Poll, Registry, Token, event::Source, net::UdpSocket as MioUdpSocket};
...
impl Zeroconf {
    ...
    fn run(&mut self, receiver: Receiver<Command>) -> Option<Command> {
        ...
        if let Some(sock) = self.ipv4_sock.as_mut()
            && let Err(e) = self.poller.registry().register(
                sock,
        ...
        loop {
            ...
            for hostname in self.hostname_resolvers.keys() {
    ...
    fn del_addr_in_my_services(&mut self, addr: &IpAddr) {
        for service_info in self.my_services.values_mut() {
    ...
    fn check_ip_changes(&mut self) {
        ...
        for (if_index, last_ipv4, last_ipv6) in deleted_intfs {
            ...
            if let Some(ipv4) = last_ipv4 {
                ...
                if let Some(sock) = self.ipv4_sock.as_mut()
                    && let Err(e) = sock.pktinfo.leave_multicast_v4(&GROUP_ADDR_V4, &ipv4)
                {
            ...
            match intf.addr.ip() {
                IpAddr::V4(ipv4) => {
                    if my_intf.next_ifaddr_v4().is_none()
                        && let Some(sock) = self.ipv4_sock.as_mut()
                    {
                ...
                IpAddr::V6(ipv6) => {
                    if my_intf.next_ifaddr_v6().is_none()
                        && let Some(sock) = self.ipv6_sock.as_mut()
                        && let Err(e) = sock.pktinfo.leave_multicast_v6(&GROUP_ADDR_V6, if_index)
                    {
    ...
    fn probing_handler(&mut self) {
        ...
        for (if_index, intf) in self.my_intfs.iter() {
            ...
            if !out.questions().is_empty() {
                ...
                if let Some(sock) = self.ipv4_sock.as_mut()
                    && let Err(InternalError::IntfAddrInvalid(intf_addr)) =
        ...
        if let Some(records) = self.cache.get_srv(instance) {
            for record in records {
                if let Some(srv) = record.record.any().downcast_ref::<DnsSrv>()
                    && self.cache.get_addr(srv.host()).is_none()
                {
    ...
    fn resolve_service_from_cache(
        ...
        if let Some(records) = self.cache.get_srv(fullname)
            && let Some(answer) = records.iter().find(|r| !r.record.expires_soon(now))
            && let Some(dns_srv) = answer.record.any().downcast_ref::<DnsSrv>()
        {
        ...
        if let Some(records) = self.cache.get_txt(fullname)
            && let Some(record) = records.iter().find(|r| !r.record.expires_soon(now))
            && let Some(dns_txt) = record.record.any().downcast_ref::<DnsTxt>()
        {
        ...
        for answer in msg.answers().iter() {
            ...
            if answer.get_type() == RRType::A || answer.get_type() == RRType::AAAA {
                if let Some(answer_addr) = answer.any().downcast_ref::<DnsAddress>()
                    && answer_addr.interface_id.index != if_index
                {
                    debug!(
    ...
    fn handle_query(&mut self, msg: DnsIncoming, if_index: u32, querier_addr: SocketAddr) {
        ...
        for question in msg.questions().iter() {
            ...
            if qtype == RRType::PTR {
                ...
                if qtype == RRType::ANY
                    && msg.num_authorities() > 0
                    && let Some(probe) = dns_registry.probing.get_mut(q_name)
                {
        ...
        for (ty_domain, sender) in self.service_queriers.iter() {
            if let Some(instances) = expired.get(ty_domain) {
                for instance_name in instances {
                    ...
                    match sender.send(event) {
                        Ok(()) => debug!(
                            "notify_service_removal: sent ServiceRemoved to listener of {ty_domain}: {instance_name}"
                        ),
    ...
    fn refresh_active_services(&mut self) {
        ...
        for ty_domain in self.service_queriers.keys() {
...
fn resolve_addr_to_index(if_kind: IfKind, interfaces: &[Interface]) -> IfKind {
    if let IfKind::Addr(addr) = &if_kind
        && let Some(intf) = interfaces.iter().find(|intf| &intf.ip() == addr)
    {
        let if_index = intf.index.unwrap_or(0);
...
mod tests {
    use super::{
        _new_socket_bind, DaemonEvent, GROUP_ADDR_V4, HostnameResolutionEvent,
        INITIAL_QUERY_DELAY_MAX_MILLIS, INITIAL_QUERY_DELAY_MIN_MILLIS, IfKind,
        MAX_PKT_ABSOLUTE_IPV6, MAX_PKT_DEFAULT, MDNS_PORT, MIN_MAX_PACKET_SIZE,
        MaxPacketSizeSelection, MyIntf, SHARED_RESPONSE_DELAY_MAX_MILLIS,
        SHARED_RESPONSE_DELAY_MIN_MILLIS, SendConfig, ServiceDaemon, ServiceEvent, ServiceInfo,
        check_domain_suffix, check_service_name_length, hostname_change, my_ip_interfaces,
        name_change, resolve_max_packet_size, send_dns_outgoing_impl, valid_instance_name,
        valid_ip_on_intf,
    };
    use crate::{
        dns_parser::{
            CLASS_IN, DnsEntryExt, DnsIncoming, DnsOutgoing, DnsPointer, FLAGS_AA, FLAGS_QR_QUERY,
            FLAGS_QR_RESPONSE, InterfaceId, RRType, ScopedIp,
        },

...
v2 帧头在 WebSocket 上取 `seq=0, frag_cnt=0` 时与 v1 语义等价（观看端页面与服务端同源部署，升级整体替换，不做线上双版本转换）。
...
| `/api/webrtc/start` | POST | WebRTC 观看信令①：`{ "streamId" }` → `{ "peerId", "sdp" }`（标准 SDP offer） |
| `/api/webrtc/answer` | POST | WebRTC 观看信令②：`{ "peerId", "sdp" }`（观看端 answer） |
| `/ws/push` | WS | 推流端点 |
| `/ws/watch?stream=ID` | WS | 观看端点 |

> ① WebRTC 观看端（设计文档阶段 1）：中继为每个观看端创建一个 peer，
> 含 `control`（可靠、有序，控制消息）与 `media`（不可靠、乱序，媒体帧）两个
> data channel；媒体帧与 WS 线格式完全一致（24 字节 v2 帧头 + 载荷）。
> 观看端页面优先 WebRTC（低延迟），失败/超时自动回退 WS。

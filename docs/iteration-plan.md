...
| 第三十一轮 | GUI 接收面板**帧率统计**（播放器可观测性）：链路行同屏显示「解码 ~Nfps · 显示 ~Nfps」（poll 差分按实际间隔归一化 + 最近 4 次滑动平均，瞬时差分 0 不抹历史）；把第三十轮排查结论（「解码帧率高、播放帧率低」= 显示管线瓶颈）落地为直接可见指标——修复后两者趋近，用户无需再靠猜；jsdom [3e] 新增持续流帧率断言（10fps 模拟，解码/显示 N>0） |
...
- **推流并发化**：`engines: HashMap<stream_id, RunningStream>`；接收端多链路已落地（Phase C，见第二十八轮）。

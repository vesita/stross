...
function notifyUnsubscribe(link) {
    if (!link.negotiateHost || !link.negotiatePort || !link.endpointId)
        return;
    ...
    if (!localNodeId || Number.isNaN(endpointId))
        return;
    const body = {
        nodeId: localNodeId,
        endpointKind,
...
async function hideActiveSurface() {
    if (!IS_ANDROID)
        return;
    try {
...
async function syncAndroidSurfaceBounds() {
    if (!IS_ANDROID)
        return;

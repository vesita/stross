...
//!
//! 每条流（[`StreamId`]）一个**解读模块**（pick 规则层，docs/framework-v3.md
//! §3.7）：
//!
//! * [`RealtimePacing`](crate::RealtimePacing)（默认）：媒体帧严格即时解读
//!   ——按传输可靠性分流（无损直通 / 有损经 [`JitterBuffer`](crate::JitterBuffer)
//!   按序/按关键帧对齐产出）；
//! * [`StrictOrdered`](crate::StrictOrdered)：文件分块 / 剪贴板 / 输入——
//!   严格顺序、逐字节不丢（Lossless + seq 单调校验）。
//!
//! [`InterpretRegistry`] 是流级容器：以强类型 [`StreamId`] 装载/索引各解读
//! 模块，生命周期与流一致（`adapter()` 于流建立、`remove()` 于流拆除）。
//!
//! 通道分流由调用方决定：`ChannelKind` 由传输可靠性契约计算后传入
//! （stross-pick 不依赖 transport——`channel_kind_for_url` 留在 kernel）。

use std::collections::{HashMap, VecDeque};
use std::time::{Duration, Instant};

use stross_proto::frame::{Frame, TRACK_VIDEO};
use stross_proto::message::{PickRule, StreamId};

use crate::buffer::{JitterBuffer, JitterConfig};
use crate::{Interpreter, RealtimePacing, StrictOrdered};

/// 流式通道的数据路径（由传输可靠性契约决定，调用方计算后传入）。
...
/// 解读模块注册表：按流装载 pick 规则解读模块。
#[derive(Default)]
pub struct InterpretRegistry {
    interpreters: HashMap<StreamId, Box<dyn Interpreter>>,
}

impl InterpretRegistry {
    /// 流建立时创建（或复用）其解读模块。
    ///
    /// `rule`：协商定稿的 pick 规则（订阅握手携带）——
    ...
    pub fn adapter(
        ...
        // 热路径快速查找：StreamId 底层 SmolStr 哈希廉价，避免每包构造 String
    ...
    /// 流拆除时移除（释放缓冲）。
    pub fn remove(&mut self, session_id: &StreamId) {
...
mod tests {
    use super::*;
    use stross_proto::frame::{CODEC_AAC, CODEC_H264, FLAG_KEYFRAME, TRACK_AUDIO, TRACK_VIDEO};
    ...
    fn manager_loads_adapter_by_rule() {
        ...
        assert!(m.is_empty());
        let s1 = StreamId::from("sess-1");
        let s2 = StreamId::from("sess-2");
        // 严格即时规则（默认/媒体）：RealtimePacing 模块
        m.adapter(&s1, PickRule::Realtime, ChannelKind::Lossy)
            .push(frame(TRACK_AUDIO, 0, false));
        // 严格顺序规则（文件/剪贴板）：StrictOrdered 模块
        m.adapter(&s2, PickRule::StrictOrdered, ChannelKind::Lossless)
            .push(frame(TRACK_AUDIO, 0, false));
        assert_eq!(m.len(), 2);
        assert_eq!(
            m.adapter(&s1, PickRule::Realtime, ChannelKind::Lossy)
    ...
    fn manager_lifecycle() {
        ...
        m.adapter(&s1, PickRule::Realtime, ChannelKind::Lossy)
            .push(frame(TRACK_AUDIO, 0, false));
        m.adapter(&s2, PickRule::Realtime, ChannelKind::Lossless);
        assert_eq!(m.len(), 2);
        m.remove(&s1);
        assert_eq!(m.len(), 1);
        assert!(

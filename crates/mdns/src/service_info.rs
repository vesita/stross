...
pub(crate) fn decode_txt(txt: &[u8]) -> Vec<TxtProperty> {
    ...
    while offset < txt.len() {
        ...
        if offset_end > txt.len() {
            debug!(
                "DNS TXT record contains invalid data: Size given for property would be out of range. (offset={}, length={}, offset_end={}, record length={})",
                offset,
                length,
                offset_end,
                txt.len()
...
impl DnsRegistry {
    ...
    pub(crate) fn update_hostname(
        ...
        for probe in self.probing.values_mut() {
            probe.records.retain(|record| {
                if record.get_type() == RRType::SRV
                    && let Some(srv) = record.any().downcast_ref::<DnsSrv>()
                    && srv.host() == original
                {
...
#[cfg(test)]
mod tests {
    use super::{DnsRegistry, ServiceInfo, TxtProperty, decode_txt, encode_txt, u8_slice_to_hex};
    use crate::dns_parser::{CLASS_IN, DnsOutgoing, DnsPointer, FLAGS_QR_RESPONSE, RRType};

...
3. **代码兜底（已实现）**：既然该网络**单播双向是通的**（§8.2 证据），`scan_lan` 增加**子网单播扫描回退**——当 mDNS 浏览零远端设备时，对本机各 /24 网段主机并发单播探测中继入口端口（18777/8777），命中即聚合。纯单播、不依赖组播/广播，故在「收不到下行多播」的网络仍能发现/被发现；只在 mDNS 零结果才扫描，避免每次刷新打满网卡。实现见 `crates/stross-kernel/src/devices.rs::subnet_scan`。

...
use crate::error::{Error, Result, e_fmt};
use crate::service_info::{DnsRegistry, MyIntf, ServiceInfo, is_unicast_link_local};
...
fn decode_txt(txt: &[u8]) -> Vec<TxtProperty> {
    ...
    while offset < txt.len() {
        ...
        if offset_end > txt.len() {
            trace!(
                "ERROR: DNS TXT: size given for property is out of range. (offset={}, length={}, offset_end={}, record length={})",
                offset,
                length,
                offset_end,
                txt.len()
...
impl DnsOutPacket {
    ...
    fn write_question(&mut self, question: &DnsQuestion) -> WriteResult {
        ...
        self.write_name(&question.entry.name).inspect_err(|_e| {
...
impl<'a> PacketBuilder<'a> {
    ...
    fn finish(mut self) -> Vec<DnsOutPacket> {
        ...
        if self.out.is_query()
            && let Some((_last, rest)) = packets.split_last_mut()
        {
            for packet in rest {
...
impl DnsOutgoing {
    ...
    const fn wire_id(&self) -> u16 {
        if self.multicast { 0 } else { self.id }
    }
...
mod tests {
    use super::{
        CLASS_CACHE_FLUSH, CLASS_IN, DnsAddress, DnsHostInfo, DnsIncoming, DnsOutPacket,
        DnsOutgoing, DnsPointer, DnsTxt, FLAGS_QR_QUERY, FLAGS_QR_RESPONSE, FLAGS_TC,
        MAX_PKT_ABSOLUTE_IPV6, MAX_PKT_DEFAULT, MSG_HEADER_LEN, RRType,
    };

...
//! * 媒体端点（屏幕 / 麦克风）→ 复用 [`StrossApp::start_stream`]：pull 推本机
...
pub fn install_endpoint_driver(app: &Arc<StrossApp>) {
    let a = app.clone();
...
fn resolve_push_url(app: &StrossApp, ctx: &SubscribeCtx) -> Option<String> {
    match ctx.delivery {
        Delivery::Push => {
...
fn resolve_watcher_base(app: &StrossApp, ctx: &SubscribeCtx) -> Option<String> {
    match ctx.delivery {
...
fn spawn_file_push(app: Arc<StrossApp>, endpoint_id: String, ctx: SubscribeCtx) {
...
fn spawn_media_push(app: Arc<StrossApp>, endpoint_id: String, ctx: SubscribeCtx, kind: MediaKind) {
    tokio::spawn(async move {
